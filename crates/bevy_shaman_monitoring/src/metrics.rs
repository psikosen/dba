...
pub fn init_metrics() {
    unsafe {
        ...
        let frame_time = Histogram::with_opts(
            HistogramOpts::new("bevy_shaman_frame_time_seconds", "Frame rendering time")
                .buckets(vec![
        ...
        let fps =
            Gauge::with_opts(Opts::new("bevy_shaman_fps", "Current frames per second")).unwrap();
...
pub fn update_performance_metrics(time: Res<Time>, diagnostics: Res<bevy::diagnostic::DiagnosticsStore>) {
...
pub fn update_game_metrics(
    // Add your game-specific queries here
    // For example:
